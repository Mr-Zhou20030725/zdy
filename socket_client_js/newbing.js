...
const addTextToTextArea = (message) => {
  ...
  if (textAreaElement) {
    ...
    setTimeout(function () {
      let button = document.querySelector('.absolute.p-1.rounded-md.md\\:bottom-3.md\\:p-2.md\\:right-3.dark\\:hover\\:bg-gray-900.dark\\:disabled\\:hover\\:bg-transparent.right-2.disabled\\:text-gray-400.enabled\\:bg-brand-purple.text-white.bottom-1\\.5.transition-colors.disabled\\:opacity-40');
      button.click();
    }, 200);
  }
};
...
const getTextContent = (element) => {
  messageQueue = [];
  let elements = document.querySelectorAll('.min-h-\\[20px\\].flex.items-start.overflow-x-auto.whitespace-pre-wrap.break-words.flex-col.gap-4');
  var i = 0;
  elements.forEach(element => {
    if (i % 2 == 0) {
      console.log("people:" + element.innerText);
    }
    else {
      messageQueue.push(element.innerText);
      console.log("robot:" + element.innerText);
    }
    i++;
  });
